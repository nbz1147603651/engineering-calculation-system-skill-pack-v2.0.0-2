...
description: 工程计算系统全生命周期专家。评估参考资料缺失、搜索获取规范标准、转化为计算逻辑蓝图、创建实施交接契约、构建可审计计算书软件、验证公式报告批量流可追溯性。Use proactively when any engineering calculation task is mentioned.
---
...
第三阶段（实现验证）：08 架构 → 09 数据模型 → 10 模块 → 11 运行器 → 12 接口 → 13 验证
```
...
**12 接口**：报告生产决策 → 输入映射 → ReportContext → 批量流 → 冒烟测试
...
| 12 接口 | `skills/12-report-review-batch-interfaces.skill.md` |
...
show_widget(
  ...
  data: {
    ...
    tasks: [
      {id: "T1", title: "任务描述", status: "pending|in_progress|done", phase: 1}
    ]
  },
  title: "工程计算系统控制台",
