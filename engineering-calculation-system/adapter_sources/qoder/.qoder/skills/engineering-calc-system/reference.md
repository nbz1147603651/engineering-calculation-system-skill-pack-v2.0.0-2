...
生产 UI 必须使用统一 UI Kit：`templates/implementation/ui_design_system.md`、`webapp/templates/partials/_topbar.html`、`webapp/templates/partials/_report_modal.html`、`webapp/static/css/tokens.css`、`webapp/static/css/components.css`。不得随机更换整体布局、顶部操作位置、语言切换、状态条或报告弹窗。

计算书最终导出应先由智能体/项目自动选择：本地存在 `latexmk` 或 `pdflatex` 时选择 LaTeX/PDF，必须实际编译，退出码为 0 且生成 `main.pdf` 才算完成；本地没有 LaTeX 编译器时使用 A4 HTML。HTML 必须是正式计算书版式，包含 `@page size: A4`、严谨页边距、封面/标题区、控制摘要、输入摘要、验算表、公式逻辑追溯、警告错误、追溯元数据和模板边界说明。计算书导出还应支持 LaTeX/Overleaf 兼容 zip：初始化或首次导出时必须询问用户是否有指定模板、`.cls/.sty`、公司封面、页面格式或章节顺序；用户未回复、明确没有或未提供可用模板时，记录为 `no_response_default` 或 `user_declined`，并使用 `latex/templates/default_engineering_calcbook/`。生成的 Web 项目必须提供 `GET /api/report/decision`、`GET /api/report/templates`、稳定的 `latex_template_id` 选择控件、`POST /api/report/final` 自动最终导出，以及 `POST /api/report/latex` 的默认回退逻辑；模板只展示 BookResult/ReportContext，不进行工程计算。
