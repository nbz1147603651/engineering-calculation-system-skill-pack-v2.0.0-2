...
生产 Web UI 必须使用统一 UI Kit：`templates/implementation/ui_design_system.md`、`webapp/templates/partials/`、`webapp/static/css/tokens.css`、`webapp/static/css/components.css`。不得随机更换整体布局、顶部操作位置、语言切换、状态条或报告弹窗。

计算书最终导出先自动选择：检测到本地 `latexmk` 或 `pdflatex` 时走 LaTeX/PDF 并实际编译，退出码为 0 且生成 `main.pdf` 才算完成；未检测到时走 A4 HTML。HTML 必须包含 `@page size: A4`、公式逻辑追溯和模板边界说明。计算书导出也应支持 LaTeX/Overleaf 兼容 zip。初始化项目或首次生成时必须询问用户是否有指定模板、`.cls/.sty`、封面、页面格式或章节顺序；用户未回复、明确没有或未提供可用模板时，记录为 `no_response_default` 或 `user_declined`，并使用默认 `latex/templates/default_engineering_calcbook/`。生成的 Web 项目必须提供 `GET /api/report/decision`、`GET /api/report/templates`、稳定的 `latex_template_id` 选择控件、`POST /api/report/final`，并通过 `/api/report/latex` 导出；当 `latex_template_id` 缺失或为空时后端必须回退默认模板。
