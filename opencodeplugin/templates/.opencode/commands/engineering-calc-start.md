...
If available, call `engineering_calc_route` with phase `router`. Then load:
...
For explicit multi-agent or parallel work, call `engineering_calc_route` with `phase=orchestration` and `parallel=true`, then use `engineering_calc_orchestration` for draft planning artifacts.
